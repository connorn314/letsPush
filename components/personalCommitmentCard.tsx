...
const PersonalCommitmentCard = ({ item, onPress, hideShadow }: {item: Workout; onPress: () => void; hideShadow?: boolean; }) => {
    const date = item.startDate.toDate();
    return (
        <TouchableOpacity  className={`${hideShadow ? "" : "bg-[#ffffff] shadow-sm"} border-rounded-full w-full rounded-lg p-4 my-1 flex-row justify-start items-center`}
            onPress={onPress}  >
            ...
            <View className="ml-2">
                <Text className="text-lg font-medium">{item.name || `${Math.round(item.distance)} mile run`}</Text>
